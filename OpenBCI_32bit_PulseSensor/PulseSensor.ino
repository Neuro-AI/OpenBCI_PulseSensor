...
//int pulsePin = 12;                // Pulse Sensor purple wire connected to pin D12, which can be analog
//int blinkPin = 11;                // pin to blink led at each beat connected to D11
...
//unsigned long pulseSampleCounter = 0;          // used to determine pulse timing
...
void getPulse(){
  ...
  if (N > 250){                                   // avoid high frequency noise
    if ( (Signal > thresh) && (Pulse == false) && (N > (IBI/5)*3) ){  // && (Signal-T > noise)       
      ...
      digitalWrite(blinkPin,HIGH);                // turn on pin 11 LED
  ...
  if (Signal < thresh && Pulse == true){   // when the values are going down, the beat is over
    digitalWrite(blinkPin,LOW);            // turn off pin 11 LED
